...
public class InterstellarPassagePlayer : MonoBehaviour
{
    ...
    public void GetHit()//受到撞击
    {
       // Debug.Log("受到撞击");
        ...
        UpdateBar();
        invincible = true;
        gameObject.GetComponent<Animator>().SetTrigger("hit");//播放受击动画
        if(HP <= 0) 
        {
            HP = 0; 
            stop = true;
    ...
    private void UpdateBar()//更新条长度
    {
        ...
        EnergyBar.transform.localPosition = new Vector3((energy / 24f)*300-300, 0,0);
        if(energy/24f < 0.25)
        {
        ...
        HPBar.transform.localPosition = new Vector3((HP / 3f) * 300 - 300, 0, 0);
        if (HP / 3f < 0.35)
        {
